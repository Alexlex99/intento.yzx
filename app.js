...
grunt.registerTask('Transformar' , function obUsu (req, res, next){
var options = this.options({entrada: 'datos.json', salida: 'datos.json'});
var values=[1];
var text= 'SELECT mail, nombre, apellido, direccion, num_int FROM usuario where';
client.query(text, ( err, respuesta)=>{
	if (err) {
			console.log('ERROR: ',err);
			return;
		}else{
